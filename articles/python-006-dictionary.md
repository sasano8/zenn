...
辞書にフォーカスした解説のため、基礎的な知識・単語については、ほかの記事等で補完ください。
...
Python3.7未満は順序不定ですので、挿入順序を管理する場合は[`OrderedDict`](#順序性を保持した辞書を作成する)を利用ください。
...
標準では機能が用意されていませんが、次のように`__setitem__`をオーバーライドすることで実現可能です。
...
| 登録 | `dic["name"] = val` | キーが存在しない場合、`KeyError`が発生 | |
| 登録/取得 | `dic.setdefault("key")` | キーが存在しない場合、`None`を登録したうえで、キーの値を返す | |
| 登録/取得 | `dic.setdefault("key", None)` | キーが存在しない場合、第２引数の値を登録したうえで、キーの値を返す | |
| 取得 | `dic["name"]` | キーが存在しない場合、`KeyError`が発生 | |
