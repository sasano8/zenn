...
辞書は基礎的なオブジェクトですが、度々機能改善されたり、発展途上の側面があります。

筆者も日々様々な辞書操作について発見があり、自分なりのノウハウをメモしているうちに、全部まとめてしまえと執筆したのが本記事です。

本記事で紹介する辞書操作や挙動は、本記事[まとめ](#まとめ)の章でコンパクトにまとめました。
本記事を一通りお読みいただいた後は、ガイドラインとしてまとめを活用いただければ幸いです。
...
性能面でも優れている最もポピュラーな方法なので、基本的にこの方法を使用しましょう。
``` Python
dic = {"key": "test"}
```
...
キーに、ダブルクォーテーションを打たなくてよいため、タイプ数が少なくなります。

ただし、`dict`関数を使用する場合、以下のような言語仕様上の制約があります。
...
`dict`関数にキーと値のタプルをリストにして渡し、辞書を作成できます。
...
- 第１引数：　キー
- 第２引数：　キーが登録されていない場合に登録する値（省略時はNone）
:::
...
`KeyError`を無視したい場合は、`get`メソッドを使用しましょう。

:::message
`get`は、指定したキーの値を返します。
指定したキーが存在しない場合は、第２引数の値を返します。
- 第１引数：　キー
- 第２引数：　キーが登録されていない場合に返す値（省略時は`None`）
:::
...
ただし、`setdefault`メソッドはデフォルト値を登録したいケースで利用しましょう。
...
- 第１引数：　キー
- 第２引数：　キーが登録されていない場合に返す値（省略時は`KeyError`が発生）
:::
...
### popitemを使用して削除する
`popitem`メソッドは、要素を１つ無作為に削除します（Python3.7からは辞書の要素に順序が存在するため、最後から削除されると思われます。）。
キーを指定できず、何が削除されるか分からないため、使う機会はほぼないでしょう。

:::message
`popitem`メソッドは、要素を１つ無作為に削除します。
...
## デフォルトの順序について
Python3.7からは、辞書が返す要素の順序は**挿入順**（更新は順序に影響を与えません）になりました。
https://docs.python.org/3/library/stdtypes.html#dict-views

Python3.7未満は順序不定なので、挿入順序を管理する場合は[`OrderedDict`](#順序性を保持した辞書を作成する)を利用してください。
...
### Python3.5 未満
`dict`関数の第１位置引数にソース辞書を渡し、アンパック記法を併用することでマージされた辞書を作成できます。
...
### Python3.9以上
Python3.9からは、和集合演算子`|`と累算代入演算子`|=`が導入され、マージする意思をより明示的に表現可能になりました。
...
| 登録 | `dic["name"] = val` | キーが存在しない場合、KeyErrorが発生 | |
